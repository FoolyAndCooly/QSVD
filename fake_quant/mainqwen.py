...
def _profile(dataloader, args, model, image_processor, tokenizer, out=False):
    ...
    inps = torch.zeros(
        (args.vitnsamples, model.model.visual.num_patches, model.model.visual.config.hidden_size), dtype=dtype, device=utils.get_dev()
    )
    inps = []
    outs = torch.zeros(
        (args.vitnsamples, model.model.visual.num_patches, model.model.visual.config.hidden_size*5), dtype=dtype, device=utils.get_dev()
    ) if out else None
    ...
    model.model.visual = model.model.visual.to(utils.get_dev())
    layer = model.model.visual.merger.mlp
...
def main(args):
    ...
    model.model.visual.to(modeldtype)
    print(model.model.visual.dtype)
    ...
    if args.vit_module:
        ...
        config = model.model.visual.config
    ...
    if not args.lm_off:
        ...
        if args.rotate:
            ...
            quant_utils.add_actquant(model.language_model.layers) #Add Activation Wrapper to the model as the rest of the code assumes it is present
    ...
    if 'scienceqa' in args.eval_dataset.lower():
        import eval_utilsdistqwen
        dataset_ppl = eval_utilsdistqwen.evaluator(model, testloader, utils.get_dev(), args, tokenizer, image_processor)
